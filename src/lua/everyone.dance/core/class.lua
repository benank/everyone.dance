...
function ED.generate_init_function(class_instance, args)
    return
    function()
...
local function create_class(...)
    ...
    setmetatable(cls, {__call = function (c, ...)
        ...
        if not instance.tostring then -- if the class doesnt have a :tostring() method, add one that spits out a warning if it gets called
            instance.tostring = function() return "everyone.dance Warning: called tostring on a class instance when :tostring() was not implemented" end
        end
